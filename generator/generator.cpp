...
int readCuboid(FILE *fp,const char *endcommand)
{
   // char buf[1024];
    ...
    Cuboid_t cub;
    int spaceflag=0;
    int typeflag=0,rigidtypeflag=0,lowerflag=0, upperflag=0, veloflag=0,enthalpyflag=0,angleflag=0;
    cub.type = iType_t(0);
    ...
    while(1){
        ...
        if(strcmp(token,endcommand)==0)break;
            ...
            enthalpyflag=1;
                ...
                if(fscanf(fp,"%lf",&cub.angle)!=1)goto err;
            angleflag=1;
        }
        else{
    ...
    if(!(spaceflag && typeflag && rigidtypeflag   && lowerflag && upperflag && veloflag  && enthalpyflag && angleflag  ))return 1;
        fprintf(stderr, "line %d\n", __LINE__);
    Cuboid += cub;
    return 0;
...
//    char buf[1024];
    ...
    Cuboid2_t cub2;
    int spaceflag=0;
    int typeflag=0, lowerflag=0, upperflag=0, veloflag=0, enthalpyflag=0,angleflag=0;
    cub2.type = iType_t(0);
...
//    char buf[1024];
    ...
    while(1){
        ...
        if(strcmp(token,endcommand)==0)break;
            ...
            if(fscanf(fp,"%d",&rec.rigidtype.setvalue())!=1)goto err;
            rigidtypeflag=1;
        }
        else if(strcmp(token,"Lower")==0){
            for(int iDim=0;iDim<3;++iDim){
...
for(iCub_t iCub(0); iCub < cubcount; ++iCub){
    ...
    const vec3<int> count = vec3<int>(
        round(width[0] / cub.space),
        round(width[1] / cub.space),
        round(width[2] / cub.space)
    );
    ...
    // 回転角（Z軸まわり，単位：ラジアン）
    const double angle = cub.angle*M_PI/180.0;
    const double cosA = cos(angle);
    const double sinA = sin(angle);

    // 中心点（回転の基準）
    const vec_t center = 0.5 * (cub.lower + cub.upper);

    for(double px = cub.lower[0] + 0.01 * spacing[0]; px < cub.upper[0] - 0.0 * spacing[0]; px += spacing[0]){
        for(double py = cub.lower[1] + 0.01 * spacing[1]; py < cub.upper[1] - 0.0 * spacing[1]; py += spacing[1]){
            for(double pz = cub.lower[2] + 0.01 * spacing[2]; pz < cub.upper[2] - 0.0 * spacing[2]; pz += spacing[2]){

                // 中心基準でローカル座標を定義
                double lx = px - center[0];
                double ly = py - center[1];
                double lz = pz - center[2]; // zは回転しない
                ...
                // 回転後のワールド座標
                vec_t position = vec_t(rx + center[0], ry + center[1], rz + center[2]);
...
for(iCub2_t iCub2(0); iCub2 < cubcount2; ++iCub2){
    ...
    // 回転角（Z軸回転，単位：ラジアン）
    const double angle = cub2.angle*M_PI/180.0;
    const double cosA = cos(angle);
    const double sinA = sin(angle);

    // 回転の中心（直方体の中心）
    const vec_t center = 0.5 * (cub2.lower + cub2.upper);
    ...
    for(double px = cub2.lower[0] + 0.01 * spacing[0]; px < cub2.upper[0] - 0.0 * spacing[0]; px += spacing[0]){
        for(double py = cub2.lower[1] + 0.01 * spacing[1]; py < cub2.upper[1] - 0.0 * spacing[1]; py += spacing[1]){
            for(double pz = cub2.lower[2] + 0.5 * spacing[2]; pz < cub2.upper[2] - 0.49 * spacing[2]; pz += spacing[2]){
                ...
                double lx = px - center[0];
                double ly = py - center[1];
                double lz = pz - center[2]; // z軸は回転しない
...
for (iCyb_t iCyb(0); iCyb < cybcount; ++iCyb) {
    ...
    const vec_t center = 0.5 * (cyb.upper + cyb.lower); // Center of the bounding box
    ...
    // Loop through the grid points within the bounding box
    for (double px = cyb.lower[0] + 0.01 * spacing[0]; px < cyb.upper[0] - 0.0 * spacing[0]; px += spacing[0]) {
        for (double py = cyb.lower[1] + 0.01 * spacing[1]; py < cyb.upper[1] - 0.0 * spacing[1]; py += spacing[1]) {
            for (double pz = cyb.lower[2] + 0.01 * spacing[2]; pz < cyb.upper[2] - 0.0 * spacing[2]; pz += spacing[2]) {
                ...
                // Calculate r^2 in cylindrical coordinates (x^2 + y^2)
                double r_squared = x * x + y * y;//+ z*z;

                // Calculate inner and outer radius squared based on ratio and width
                double inner_radius_squaredx = pow(0.5 * width[0] * cyb.ratio, 2); // Inner radius scales with cyb.ratio
                double outer_radius_squaredx = pow(0.5 * width[0], 2);              // Outer radius is half of width[0]
                double inner_radius_squaredy = pow(0.5 * width[1] * cyb.ratio, 2); // Inner radius scales with cyb.ratio
                double outer_radius_squaredy = pow(0.5 * width[1], 2);              // Outer radius is half of width[0]
                double inner_radius_squaredz = pow(0.5 * width[2] * cyb.ratio, 2); // Inner radius scales with cyb.ratio
                double outer_radius_squaredz = pow(0.5 * width[2], 2);    

                // If within the cylinder between inner and outer radii, process point
                if (r_squared > inner_radius_squaredx && r_squared <= outer_radius_squaredx &&r_squared > inner_radius_squaredy && r_squared <= outer_radius_squaredy){ //&&r_squared > inner_radius_squaredz && r_squared <= outer_radius_squaredz) {
                    fprintf(stderr, "p %e %e %e\n", px, py, pz);
                    Type += cyb.type;
...
for(iRec_t iRec(0); iRec < reccount; ++iRec){
    ...
    const double angle_rad = rec.angle * M_PI / 180.0;

    const vec3<int> count = vec3<int>(
        round(width[0] / rec.space),
        round(width[1] / rec.space),
        round(width[2] / rec.space)
    );
    const vec_t spacing = vec_t(
        width[0] / count[0],
        width[1] / count[1],
    ...
    const double x0 = rec.lower[0];
    const double y0 = rec.lower[1];
    const double x1 = rec.upper[0];

    for(double px = x0; px <= x1; px += spacing[0]){
        // 三角形の上端は tan(angle) によって決まる
        double max_py = tan(angle_rad) * (px - x0) + y0;

        for(double py = y0; py <= max_py; py += spacing[1]){
            for(double pz = rec.lower[2]; pz < rec.upper[2]; pz += spacing[2]){
                
                // この範囲は三角形内
                fprintf(stderr, "p %e %e %e\n", px, py, pz);

                Type      += rec.type;
                RigidType += rec.rigidtype;
                Position  += vec_t(px, py, pz);
